...
silk_decode(PyObject* Py_UNUSED(module), PyObject* args, PyObject* keyword_args) {
    ...
    if (PyUnicode_Check(silk_obj)) {
        /* File path mode */
        bitInFile = _Py_fopen_obj(silk_obj, "rb");
        if (bitInFile == NULL) {
    ...
    } else if (PyObject_HasAttr(silk_obj, PyId_read)) {
        /* BytesIO mode: call read() to get bytes */
        silk_read_result = PyObject_CallMethodNoArgs(silk_obj, PyId_read);
        ...
        PyErr_SetString(PyExc_TypeError, "silk must be str, bytes, bytearray, or file-like object with read()");
        return NULL;
    }
    ...
    /* Check Silk header */
    {
        ...
        if (strcmp(header_buf, "\x02") != 0) {
            ...
            if (silk_from_memory) {
                ...
                counter = fread(header_buf, sizeof(char), sizeof(SILK_HEADER) - 1, bitInFile);
            }
            header_buf[sizeof(SILK_HEADER) - 1] = '\0'; /* Terminate with a null character */
            ...
            /* Tencent format */
            if (silk_from_memory) {
                if (silk_pos + (Py_ssize_t)(sizeof(SILK_HEADER_TENCENT) - 1) > silk_size) {
                    if (silk_read_result) Py_DECREF(silk_read_result);
                    return PyErr_Format(PilkError, "Error: silk data too short");
                }
                memcpy(header_buf, silk_buffer + silk_pos, sizeof(SILK_HEADER_TENCENT) - 1);
                silk_pos += sizeof(SILK_HEADER_TENCENT) - 1;
            } else {
                counter = fread(header_buf, sizeof(char), sizeof(SILK_HEADER_TENCENT) - 1, bitInFile);
            }
            header_buf[sizeof(SILK_HEADER_TENCENT) - 1] = '\0'; /* Terminate with a null character */
    ...
    if (PyUnicode_Check(pcm_obj)) {
        /* File path mode */
        speechOutFile = _Py_fopen_obj(pcm_obj, "wb");
        if (speechOutFile == NULL) {
            if (silk_read_result) Py_DECREF(silk_read_result);
            if (!silk_from_memory) fclose(bitInFile);
            return PyErr_Format(PyExc_OSError, "Error: could not open output file");
        }
    } else if (PyObject_HasAttrString(pcm_obj, PyId_write)) {
        /* BytesIO mode */
        pcm_to_bytesio = 1;
        pcm_to_memory = 1;
        ...
        if (estimated_size > 10 * 1024 * 1024) estimated_size = 10 * 1024 * 1024;  /* Max 10MB */
        ...
        PyErr_SetString(PyExc_TypeError, "pcm must be str, file-like object with write(), or None");
        if (silk_read_result) Py_DECREF(silk_read_result);
        if (!silk_from_memory) fclose(bitInFile);
        return NULL;
    }
    ...
    while (1) {
        ...
        if (totBytes < 0 || totBytes > (SKP_int16)sizeof(payload)) {
            ...
            if (silk_read_result) { Py_DECREF(silk_read_result); }
            if (!silk_from_memory) fclose(bitInFile);
            if (!pcm_to_memory) fclose(speechOutFile);
            else memory_buffer_free(&pcm_buf);
    ...
    if (pcm_to_bytesio) {
        ...
        PyObject* write_result = PyObject_CallMethodOneArg(pcm_bytesio_obj, PyId_write, bytes_obj);
        Py_DECREF(bytes_obj);
